#!/usr/bin/env Rscript
# nflTeStats.R
# Advanced TE receiving stats leaderboard (TE only)
# Args: [season] [player_name (optional, for single-player mode)]
# Sources: NFL Next Gen Stats + nflreadr player_stats
...
OUT_PNG <- file.path(OUT_DIR, "nflTeStats.png")
...
# Season-aggregate NGS (TE only)
ngs_szn <- ngs %>%
  filter(season_type == "REG", player_position == "TE") %>%
...
if (single_mode) {
  ...
  if (nrow(target_row) == 0) {
    message(paste0("Player '", PLAYER, "' not found in ", SEASON, " data (min 20 targets required)."))
    quit(status = 2)
  }
}
...
p <- p +
  labs(
    title    = paste0(SEASON, " NFL Advanced Receiving - TE"),
    subtitle = paste0("Separation vs YAC Over Expected | bubble size = targets | color = Rec EPA | min 20 targets"),
